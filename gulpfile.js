...
gulp.task('clean-theme', function() {
  return gulp.src(THEME_BUILD_DIR, {
    read: false,
    force: true
  }) // Target dir is outside CWD, use force option
    .pipe(clean())
})
...
gulp.task('install-plugins', shell.task(getPhonegapPluginCommands(), {cwd: PHONEGAP_DIR}))
