...
,
  ...
  getExternalDates(externalDateOptionAttributes, code) {
    ...
    var that = this, params = $.parseJSON(externalDateOptionAttributes.params), dates = [], date = new Date(), events = []
    ...
    $.ajax({
      ...
      success(data, status, xhr) {
        var cal = $(data), links = cal.find('a.calendar'), dates = []

        var generateEvents = function () {
          links.each(function (idx, link) {
            var regex = /^\d{1,2}$/i, text = $(link).text()
...
,
  ...
  getAvailableDates(filePath) {
    // Return all files in the folder
    var that = this, filePath = filePath || 'default', files = window.files = [], months = [
      'jan',
      'feb',
      'mar',
      'apr',
      ...
      'nov',
      'dec'
    ], dateRegex = /[a-zA-Z]+([0-9]+)([a-zA-Z]+).pdf$/, dates = [], date, events = []
...
,
  ...
  getAvailableTracks(filePath, date) {
    // Return all files in the folder
    var that = this, filePath = filePath || 'default', files = window.files = [], regex = new RegExp('([a-zA-Z]+)' + page.formatTrackDate(date) + '.pdf'), results = []
    ...
    var interval = setInterval(function () {
      if (typeof window.filesLoaded !== 'undefined' && window.filesLoaded) {
        ...
        filter = (filters.hasOwnProperty('length') && filters.length > 0) ? [
          {
            logic: 'or',
            filters: filters
          }
...
,
  ...
  init() {
    var that = this, loader = page.getLoader(), progressBar
    ...
    that.progressBar = progressBar = $('#browserProgress').kendoProgressBar({
      ...
      value: 0,
      complete(e) {
        var widget = this, productDataSource = browserDataSources.get('browser.product'), doCheckout = true, product = null, date = null

        // TODO: Cart only if product is not free display
        var productConfig = viewModel.get('product_config'), productOptions = productConfig.get('option'), cartProduct = {
        ...
        if (typeof productOptions !== 'undefined') {
          ...
          productOptions.forEach(function (value, key) {
            ...
            if (value instanceof Date) {
              ...
              cartProduct.option[key.replace('product_option_', '')] = date = [
                value.getFullYear(),
        ...
        if (!doCheckout) {
          var productDownloads = viewModel.get('product_downloads'), //dateCode,
            trackCode
          ...
          infoWindow.one('refresh', function (e) {
            var popup = infoWindow.wrapper, iframe = popup.find('iframe').first()[0], doc = iframe.contentDocument
...
,
  ...
  start() {
    ...
    that.catalog.bind('change', function (e) {
      var steps = that.steps, step = that.step, menuItem = that.menu.dataSource.at(step), type = (typeof menuItem !== 'undefined') ? menuItem.type : null, item = e.sender.select(),
        id = item.attr('data-id'), entityType = item.attr('data-entity'), productConfig, optionsConfig, productOptionId
