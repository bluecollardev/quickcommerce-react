...
class File {
  ...
  static readDir(path, callback, options) {
    ...
    window.requestFileSystem(PERSISTENT, 512000, (fileSystem) => {
      fileSystem.root.getDirectory('data/' + path, { create: true }, (dir) => {
        console.log(dir)
  ...
  static createFileDownloader(context, remoteUrl, filePath) {
    ...
    return {
      ...
      fetch: () => {
        ...
        // TODO: Check slashes
        window.resolveLocalFileSystemURL(download.filePath, (entry) => {
          console.log(entry)
          ...
          File.downloadFile(download.remoteUrl, download.filePath, (entry) => {
            //download.status = 2
