...
export default class FormHelper {
   ...
   * @returns {*}
   */
  static setMappedValue(path, data, value) {
    if (typeof data === 'undefined' || data === null) {
      return null
    }
    // Access static methods using constructor property
    ...
    if (chunks.length > 0) {
      ...
      if (isArray) {
        ...
        FormHelper.setMappedValue(chunks.join('.'), data[prop][arrIdx], value)
      } else {
        //console.log(JSON.stringify(data[prop]))
        FormHelper.setMappedValue(chunks.join('.'), data[prop], value)
      }
