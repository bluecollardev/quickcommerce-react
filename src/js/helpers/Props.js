...
class PropsHelper {
  ...
  static mergeProps(a, b, { ignore = [] } = {}) {
    ...
    const className = typeof b.className === 'string' ? b.className.split(' ') : b.className

    delete b.className

    if (a.className || className) {
      a.className = [].concat(typeof a.className === 'string' ? a.className.split(' ') : a.className).concat(className)
        .filter(Boolean)
