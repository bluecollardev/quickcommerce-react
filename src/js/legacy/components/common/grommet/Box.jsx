...
export default class Box extends Component {
  ...
  setDarkBackground () {
    ...
    this.checkBackground = checkDarkBackground(colorIndex, box,
  ...
  addPropertyClass (classes, property, options = {}) {
    ...
    if (value) {
      if (typeof value === 'string') {
        classes.push(`${elementName}--${prefix}-${this.normalize(value)}`);
      } else if (typeof value === 'object') {
        Object.keys(value).forEach((key) => {
          this.addPropertyClass(classes, key, {
            object: value, prefix: `${prefix}-${key}` });
        });
      } else {
        classes.push(`${elementName}--${this.normalize(prefix)}`);
      }
    }
  }

  backgroundContextClass (darkBackground) {
    let result;
  ...
  render () {
    ...
    this.addPropertyClass(classes, 'full');
    if (full && full.responsive === undefined) {
      // default is true for backwards compatibility sake
      classes.push(`${CLASS_ROOT}--full-responsive`);
    }
    this.addPropertyClass(classes, 'direction');
    this.addPropertyClass(classes, 'justify');
    this.addPropertyClass(classes, 'align');
    this.addPropertyClass(classes, 'alignContent',
      { prefix: 'align-content' });
    this.addPropertyClass(classes, 'alignSelf',
      { prefix: 'align-self' });
    this.addPropertyClass(classes, 'reverse');
    this.addPropertyClass(classes, 'responsive');
    this.addPropertyClass(classes, 'basis');
    this.addPropertyClass(classes, 'flex');
    this.addPropertyClass(classes, 'pad');
    this.addPropertyClass(classes, 'margin');
    this.addPropertyClass(classes, 'separator');
    this.addPropertyClass(classes, 'textAlign', { prefix: 'text-align' });
    this.addPropertyClass(classes, 'wrap');
    ...
    if (size) {
      if (typeof size === 'object') {
        Object.keys(size).forEach((key) => {
          this.addPropertyClass(classes, key, { object: size });
        });
      } else {
        this.addPropertyClass(classes, 'size');
      }
      if (size) {
    ...
    if (appCentered) {
      this.addPropertyClass(containerClasses, 'full',
        { elementName: `${CLASS_ROOT}__container` });
      if (colorIndex) {
        containerClasses.push(`${BACKGROUND_COLOR_INDEX}-${colorIndex}`);
        containerClasses.push(this.backgroundContextClass(darkBackground));
      }
      ...
      classes.push(this.backgroundContextClass(darkBackground));
    }
