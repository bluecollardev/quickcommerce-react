...
class LayerContents extends Component {
  ...
  constructor(props, context) {
    ...
    this.processTab = this.processTab.bind(this);
  ...
  componentDidMount () {
    ...
    if (onClose) {
      this.keyboardHandlers.esc = onClose;
    }
...
export default class Layer extends Component {
    ...
    if (this.originalFocusedElement && !hidden) {
      if (this.originalFocusedElement.focus) {
        ...
        setTimeout(() => {
          ...
          window.scrollTo(
            this.originalScrollPosition.left, this.originalScrollPosition.top
          );
        }, 0);
  ...
  _addLayer () {
    ...
    element.className = this.classesFromProps();
    ...
    if (grommetApps) {
      Array.prototype.slice.call(grommetApps).forEach((grommetApp) => {
        if (ariaHidden && visibleLayers.length === 0) {
          ...
          grommetApp.style.top = `-${this.originalScrollPosition.top}px`;
          grommetApp.style.left = `-${this.originalScrollPosition.left}px`;
        } else {
  ...
  _renderLayer () {
    if (this.element) {
      this.element.className = this.classesFromProps();
      const contents = (
