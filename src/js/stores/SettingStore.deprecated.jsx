...
import { File } from '../helpers/Cordova.js'
import BaseStore from './BaseStore.jsx'

class SettingStoreDeprecated extends BaseStore {
  static defaultStore = {
    id: 0,
    value: 'Default Store',
    data: {
  ...
  constructor(dispatcher, adapter) {
    ...
    this.stores = [assign({}, SettingStoreDeprecated.defaultStore)]
  ...
  getDefaultStore() {
    return SettingStoreDeprecated.defaultStore
  }
  ...
  parseStores(data) {
    ...
    stores.unshift(assign({}, SettingStoreDeprecated.defaultStore))
  ...
  parseCategories() {
    ...
    this.categories = Object.keys(obj).map(c => { // TODO: Sort array
      return {
        id: obj[c]['category_id'],
        value: obj[c]['name'],
        data: obj[c]
      } // Uses legacy API for now
    })
  }
   ...
   */
  loadConfig() {
    let that = this, configFileDir, configFileName = 'config.ini'
  ...
  parseConfigFile(reader, isFileCallback) {
    let that = this, page = that.getPage(), lineRegex = /[\r\n]+/g, configRegex = /([\w]+)\s?\=\s?([\w]+)/, isDataDir = false, dataDir = false, fileName, matches

    let lines = reader.result.split(lineRegex) // tolerate both Windows and Unix linebreaks
    let pairs, idx = 0, key, value
...
export default SettingStoreDeprecated
export { SettingStoreDeprecated }
