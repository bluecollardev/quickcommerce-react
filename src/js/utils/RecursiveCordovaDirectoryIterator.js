  ...
  var recursiveIterator = Object.create(App.Utilities.Iterator(), {
    ...
    iterate: {
      value: function () {
        ...
        window.requestFileSystem(LocalFileSystem.PERSISTENT, 512000, function (fileSystem) {
          window.resolveLocalFileSystemURL(that._rootDir, function (directoryEntry) {
    ...
    dirSuccess: {
      value: function (dirEntry) {
        var that = this, directoryReader = dirEntry.createReader() // Get a directory reader
    ...
    readerSuccess: {
      value: function (entries) {
        var that = this, i = 0, len = entries.length
    ...
    hasChildren: {
      value: function () {
        var data = this.data, keys = this.keys, index = this.index, //len = this.len,
          type
